...
export default function RandomTrackButton({ categoryBaseUrl }) {
  ...
  // Fetch Spotify token on mount (same as HomePage)
  useEffect(() => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      // 🟢 Enrich the song using loadTracks (same logic as HomePage)
      let enrichedTrack = [];
      await loadTracks([randomSong], spotifyToken, (data) => {
        enrichedTrack = data;
      });

      setSelectedTrack(enrichedTrack[0]); // show enriched track in modal
