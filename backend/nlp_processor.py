...
class MusicNLPProcessor:
    def __init__(self):
        ...
        self.emotion_keywords = {
            'sad': [
                'sad', 'depressed', 'down', 'blue', 'crying', 'heartbroken', 
                'lonely', 'upset', 'hurt', 'low', 'miserable', 'gloomy', 
                'devastated', 'broken', 'disappointed', 'hopeless', 'grief', 
                'sorrow', 'despair', 'melancholic', 'tearful', 'weeping'
            ],
            'happy': [
                'happy', 'joyful', 'excited', 'cheerful', 'glad', 'upbeat', 
                'positive', 'elated', 'thrilled', 'ecstatic', 'delighted', 
                'overjoyed', 'euphoric', 'blissful', 'jubilant', 'gleeful'
            ],
            'energetic': [
                'energetic', 'pumped', 'hyped', 'active', 'intense', 'powerful', 
                'strong', 'adrenaline', 'boost', 'motivation', 'energy', 
                'beast mode', 'fired up', 'charged', 'dynamic', 'vigorous'
            ],
            'romantic': [
                'love', 'romantic', 'crush', 'valentine', 'intimate', 'romance', 
                'affection', 'passion', 'relationship', 'boyfriend', 'girlfriend',
                'husband', 'wife', 'partner', 'soulmate', 'sweetheart', 
                'darling', 'beloved', 'anniversary', 'proposal', 'honeymoon'
            ],
            'relaxed': [
                'chill', 'relax', 'calm', 'peaceful', 'meditation', 'zen', 
                'quiet', 'tranquil', 'serene', 'mellow', 'soothing', 'unwind', 
                'decompress', 'breathe', 'mindful', 'ambient', 'soft', 
                'gentle', 'laid back', 'easygoing'
            ],
            'party': [
                'party', 'club', 'dancing', 'celebration', 'fun', 'wild', 
                'night out', 'dance', 'nightclub', 'disco', 'rave', 'festival', 
                'concert', 'turn up', 'lit', 'banging', 'banger', 'groove', 'vibe'
            ],
            'motivated': [
                'motivated', 'determined', 'driven', 'ambitious', 'focused', 
                'goal', 'achieve', 'success', 'hustle', 'grind', 'push', 
                'dedication', 'commitment', 'persistence', 'winner'
            ],
            'nostalgic': [
                'nostalgic', 'memories', 'remember', 'past', 'childhood', 
                'throwback', 'old times', 'reminisce', 'vintage', 'classic', 
                'retro', 'old school', 'yesteryear'
            ],
            'melancholy': [
                'melancholy', 'bittersweet', 'wistful', 'pensive', 'contemplative', 
                'reflective', 'introspective', 'thoughtful', 'somber', 'moody'
            ]
        }
        ...
        self.negative_indicators = ['bad', 'awful', 'terrible', 'horrible', 'worst', 'hate', 'suck', 'disappointing', 'frustrating']
        ...
        self.explicit_words = {
            ...
            'f*ck', 'f**k', 'sh*t', 'sh**', 'd*mn', 'b*tch', 'a**', 'h*ll',
            'fck', 'sht', 'dmn', 'btch', 'fuk', 'shyt', 'fcking', 'fking',
            # Drug references
            'weed', 'marijuana', 'cannabis', 'pot', 'high', 'stoned', 'blazed',
            'joint', 'blunt', 'bong', 'smoke', 'lit', 'dope', 'hash',
            'cocaine', 'coke', 'crack', 'meth', 'heroin', 'pills', 'molly',
            'ecstasy', 'acid', 'lsd', 'shrooms', 'xanax', 'adderall',
            # Violence references
            'kill', 'murder', 'die', 'death', 'blood', 'gun', 'shoot', 'knife',
            'violence', 'fight', 'beat up', 'assault', 'attack', 'war', 'bomb',
        ...
        self.explicit_phrase_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.explicit_phrases]
    ...
    def preprocess_text(self, text: Union[str, None]) -> str:
        ...
        try:
            ...
            contractions = {
                ...
                "they're": "they are", "don't": "do not", "won't": "will not", "can't": "cannot"
            }
    ...
    def extract_emotions(self, text: str) -> List[str]:
        ...
        if not emotion_scores:
            positive_count = sum(1 for word in self.positive_indicators if word in processed_text)
            negative_count = sum(1 for word in self.negative_indicators if word in processed_text)
        ...
        detected_emotions = [emotion for emotion, _ in 
                           sorted(emotion_scores.items(), key=lambda x: x[1], reverse=True)[:3]]
    ...
    def detect_explicit_content(self, text: str) -> Dict:
        ...
        if not text:
            return {'has_explicit': False, 'categories': [], 'words_found': [], 'phrases_found': [], 'severity': 'none'}
        ...
        try:
            ...
            if explicit_info['has_explicit']:
                categories = set()
                all_found = explicit_info['words_found'] + explicit_info['phrases_found']
                
                for item in all_found:
                    item_lower = item.lower()
                    # Categorize based on content
                    if any(word in item_lower for word in ['fuck', 'shit', 'damn', 'bitch', 'ass', 'hell']):
                        categories.add('profanity')
                    if any(word in item_lower for word in ['weed', 'high', 'stoned', 'smoke', 'drug', 'pill', 'molly']):
                        categories.add('drugs')
                    if any(word in item_lower for word in ['kill', 'murder', 'violence', 'fight', 'gun', 'blood']):
                        categories.add('violence')
                    if any(word in item_lower for word in ['sex', 'porn', 'nude', 'horny', 'hookup']):
                        categories.add('sexual')
                    if any(word in item_lower for word in ['party hard', 'party wild', 'partying']):
                        categories.add('party_explicit')
                
                explicit_info['categories'] = list(categories)
                
                # Determine severity
                severity_score = len(explicit_info['words_found']) + len(explicit_info['phrases_found']) * 2
                if severity_score >= 5:
    ...
    def get_explicit_aware_genres(self, emotions: List[str], activities: Optional[List[str]] = None, 
                                explicit_info: Optional[Dict] = None) -> List[str]:
    ...
    def process_user_message(self, message: Union[str, None]) -> Dict:
        ...
        try:
            ...
            return {
                'emotions': [],
                'activities': [],
                'explicit_content': {'has_explicit': False, 'categories': [], 'words_found': [], 'phrases_found': [], 'severity': 'none'},
                'recommended_genres': ['trending_music', 'top_music', 'developers_choice_music'],
                'confidence': 0.1,
                'error': str(e),
                'processing_time': 0,
        ...
        try:
            ...
            if emotions:
                ...
                if activities:
                    ...
                    response_parts.append(f"I can sense you're feeling {emotion_text} and looking for something for {activity_text}.")
                else:
                    response_parts.append(f"I can feel that {emotion_text} vibe from you.")
                
                # Add explicit content acknowledgment if present
                if has_explicit:
                    if explicit_severity == 'high':
                        response_parts.append("I understand you're looking for music with explicit content that matches your energy.")
                    elif explicit_severity == 'medium':
                        response_parts.append("I can include some edgier tracks in the mix.")
            
            # Confidence-based response
            if confidence > 0.7:
                response_parts.append("I'm quite confident these songs will hit the right note:")
            elif confidence > 0.5:
                response_parts.append("Here are some songs that should match your mood:")
            else:
                response_parts.append("Let me suggest some music that might work for you:")
            ...
            return {
                ...
                'follow_up': self._generate_follow_up(emotions, activities, confidence, explicit_info),
                'confidence': confidence,
                'personalization_level': 'high' if confidence > 0.6 else 'medium' if confidence > 0.3 else 'low',
                'content_warning': has_explicit and explicit_severity == 'high',
    ...
    def _generate_follow_up(self, emotions: List[str], activities: List[str], confidence: float, explicit_info: Dict = None) -> str:
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'total_genres': len(set(genre for genres in self.emotion_genre_mapping.values() for genre in genres)),
            'emotion_keywords_count': {emotion: len(keywords) for emotion, keywords in self.emotion_keywords.items()},
            'version': '2.0'
        }
